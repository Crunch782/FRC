...
function [u, v, p, c] = NSE_Solver(u,v,p,c)
    ...
    %Save Initial fields
    iters = 0;
    save(strcat('Checkpoints/',tag,'/u_',num2str(iters),'.dat'),'u','-ASCII');
    save(strcat('Checkpoints/',tag,'/v_',num2str(iters),'.dat'),'v','-ASCII');
    save(strcat('Checkpoints/',tag,'/c_',num2str(iters),'.dat'),'c','-ASCII');
